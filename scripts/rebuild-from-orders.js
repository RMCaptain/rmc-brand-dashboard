    ...
    try {
      ...
      if (rows.length === 0) {
        ...
        if (upErr) {
          ...
          // Mirror into daily_metrics_mp — replaceDay zeroes the orders
          // columns first (ads/refund columns on the same rows survive).
          try {
            const metricsMp = require('../sync/metricsMp');
            await metricsMp.replaceDay(supabase, date, 'orders',
